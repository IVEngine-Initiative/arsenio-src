...
ConVar sv_plr_exp_bullets( "sv_plr_exp_bullets", "0", FCVAR_NOTIFY | FCVAR_REPLICATED | FCVAR_CHEAT,
	"Exploding bullets." );
...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	// Clear the number of shotgun pellet hits - we use the count as the iterator
	CBaseCombatCharacter* bcc = ToBaseCombatCharacter( this );
	if (IsPlayer())
	{
		bcc->GetActiveWeapon()->m_nPelletHits = 0;
	}
	...
	// bullet's final direction can be changed by passing through a portal
		...
		if (tr.fraction != 1.0)
		{
			...
			// player hit a combat character or freshly spawned ragdoll.
			// latter matters for shotgun hit marks - if the first pellet kills, 
			// we still want to count the other pellets hitting the ragdoll as hits
			if ( IsPlayer() && tr.m_pEnt &&
				tr.m_pEnt->IsCombatCharacter() ||
				  (tr.m_pEnt->ClassMatches("prop_ragdoll") && 
				  tr.m_pEnt->GetLastThinkTick() == 0 ) )
			{

				
				// Want to be able to hear the difference between hitting an enemy and missing
				float flSoundDur;
				if (tr.m_pEnt->BloodColor() != DONT_BLEED &&
					tr.m_pEnt->BloodColor() != BLOOD_COLOR_MECH)
				{
					EmitSound( "Flesh.BulletImpact", 0.0, &flSoundDur );
				}
				else if (tr.m_pEnt->ClassMatches( "npc_hunter" ) ||
					tr.m_pEnt->ClassMatches( "npc_strider" ))
				{
				...
				// Set the last hit time for the hud hit markers
				
				CBaseCombatWeapon* pWeapon = bcc->GetActiveWeapon();
				pWeapon->m_flLastHitTime = gpGlobals->curtime;

				// set screen positions for shotgun hit markers
				if (info.m_iShots > 1 && pWeapon->m_nPelletHits < MAX_PELLET_MARKS) 
				{
					pWeapon->m_vecPelletXYZs.Set( pWeapon->m_nPelletHits, tr.endpos );
					pWeapon->m_nPelletHits++;
				}
...
#endif
#ifndef CLIENT_DLL
				if (IsPlayer() && sv_plr_exp_bullets.GetBool())
				{
					ExplosionCreate(
						tr.endpos,
						GetAbsAngles(),
...
#ifdef GAME_DLL
		if ( bHitGlass || 
			info.m_iAmmoType == GetAmmoDef()->Index( "357" )) // Allow the 357 to penetrate anything 
		{

			HandleShotImpactingGlass( info, tr, vecDir, &traceFilter, bHitGlass );
		}
#endif
