...
bool CGameMovement::CheckJumpButton( void )
{
	...
	if (player->m_nAirJumpState == AIRJUMP_NOW)
	{
		...
		player->EmitSound(filter, player->entindex(), "Player.JumpGear");
...
ConVar sv_ladder_dampen ( "sv_ladder_dampen", "0.2", FCVAR_REPLICATED, "Amount to dampen perpendicular movement on a ladder shut the actual fuck up please you make nom fucking since and sound like a fucking bitch", true, 0.0f, true, 1.0f );
...
// Valve stop removing good fucking features. - Tux
//=============================================================================
// HPE_END
//=============================================================================
...
void CGameMovement::PlayerRoughLandingEffects( float fvol )
{
	if ( fvol > 0.0 )
	{
		...
		if (fvol > 0.25f)
		{
			CPASAttenuationFilter filter(player);
			filter.UsePredictionRules();
			player->EmitSound(filter, player->entindex(), "Player.Land");
		}
