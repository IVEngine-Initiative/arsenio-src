...
private:
	...
	float m_flBestTime; // Only for use as stopwatch
	float m_flStartTime;
...
BEGIN_DATADESC( CMathCounter )
	...
	DEFINE_FIELD(m_flBestTime, FIELD_FLOAT),
...
void CMathCounter::InputAdd( inputdata_t &inputdata )
{
	...
	if (Q_strcmp( GetEntityName().ToCStr(), "mob_stopwatch" ) == 0) {

		// Using this class as a stop watch - incrementing by timer not accurate enough
...
CFmtStr GetTimeString( float deciseconds ) {

	int dsec = floor( deciseconds );
	int sec = dsec / 10;
	int min = sec / 60;
	int hour = min / 60;
	...
	if (hour > 0)
	{
		theTime.sprintf( "%02d:%02d:%02d.%d", hour, min, sec, dsec );
	}
	else {
		theTime.sprintf( "%d:%02d.%d", min, sec, dsec );
	}
...
{
	/* Bit of a HACK to display a time for mob_speedrun.  */
	if (Q_strcmp( GetEntityName().ToCStr(), "mob_stopwatch" ) == 0)
	{
		// calculate time, converting to deciseconds
		float flTime = (gpGlobals->curtime - m_flStartTime) * 10;

		m_flBestTime = MIN( m_flBestTime, flTime );
		CFmtStr thisTime = GetTimeString( flTime );
		CFmtStr bestTime = GetTimeString( m_flBestTime );
		CFmtStr timeMsg;
		timeMsg.sprintf( "*TIME  %s\nBEST  %s*", thisTime.Access(), bestTime.Access() );
		UTIL_HudHintText( UTIL_GetLocalPlayer(), timeMsg.Access() );

		// Still need to output time
		m_OnGetValue.Set( flTime, inputdata.pActivator, inputdata.pCaller );
	}
	else
	{
