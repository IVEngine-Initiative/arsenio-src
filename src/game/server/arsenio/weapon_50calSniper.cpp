...
class CWeapon50cal : public CBaseHLCombatWeapon
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS( CWeapon50cal, CBaseHLCombatWeapon );
...
IMPLEMENT_SERVERCLASS_ST(CWeapon50cal, DT_Weapon50cal)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_50cal, CWeapon50cal );
PRECACHE_WEAPON_REGISTER(weapon_50cal);

BEGIN_DATADESC( CWeapon50cal )
...
//-----------------------------------------------------------------------------
acttable_t	CWeapon50cal::m_acttable[] = 
{
	{	ACT_RANGE_ATTACK1, ACT_RANGE_ATTACK_SNIPER_RIFLE, true }
};

IMPLEMENT_ACTTABLE(CWeapon50cal);


//-----------------------------------------------------------------------------
// Purpose: Constructor.
//-----------------------------------------------------------------------------
CWeapon50cal::CWeapon50cal( void )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CWeapon50cal::CapabilitiesGet( void ) const
{
...
//-----------------------------------------------------------------------------
bool CWeapon50cal::Holster( CBaseCombatWeapon *pSwitchingTo )
{
...
//-----------------------------------------------------------------------------
void CWeapon50cal::ItemPostFrame( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	if (pPlayer == NULL)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeapon50cal::Precache( void )
{
	BaseClass::Precache();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeapon50cal::PrimaryAttack( void )
{
	...
	if ( gpGlobals->curtime >= m_flNextPrimaryAttack )
	{
		...
		Vector vOrigin;
		GetAttachment("muzzle", vOrigin);

		CEffectData	data;

		data.m_nEntIndex = entindex();
		data.m_vOrigin = vOrigin;
		data.m_flScale = 128 * m_flPlaybackRate;
		...
		m_flNextPrimaryAttack = gpGlobals->curtime + SequenceDuration() + 1.0f;
		m_iClip1 = m_iClip1 - 1;
...
//-----------------------------------------------------------------------------
const Vector &CWeapon50cal::GetBulletSpread( void )
{
...
//			*pOperator - 
//-----------------------------------------------------------------------------
void CWeapon50cal::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch ( pEvent->event )
	{
