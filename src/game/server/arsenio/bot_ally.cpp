...
ConVar  add_jsda_prob("add_yourmom_prob", "0", 0,
...
void CBotAlly::Spawn(void)
{
	...
	if (add_jsda_prob.GetFloat() > 0)
	{
		if (RandomFloat() <= add_jsda_prob.GetFloat())
		{
...
void CBotAlly::Precache()
{
	...
	if (!Q_stricmp(pModelName, "models/Combine_dsssasSoldier.mdl"))
	{
		m_fIsElite = true;
	}
	else
