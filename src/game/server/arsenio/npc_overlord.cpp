6//========= Copyright Glitch Software, All rights reserved. ============//
//
...
// Custom function to get the player
CBasePlayer* CNPC_OverLord::GetPlayer(void)
{
	return UTIL_GetLocalPlayer();
}

// Custom function to get the player's position
Vector CNPC_OverLord::GetPlayerPosition(void)
{
...
void CNPC_OverLord::FireRocket(void)
{
	...
	Vector playerPosition = GetPlayerPosition();

	// Calculate the direction to the player
	Vector vecDirToPlayer = (playerPosition - vecOrigin);
	VectorNormalize(vecDirToPlayer);

	// Set the rocket firing angles towards the player
	VectorAngles(vecDirToPlayer, angAngles);

	// Adjust the origin to be above the OverLord
	vecOrigin.z += 50.0f;  // Adjust this value as needed

	// Create the rocket
	edict_t* pEdict = edict();
	CMissile* pRocket = CMissile::Create(vecOrigin, angAngles, pEdict);

	if (pRocket)
	{
		// Set the target for the rocket
		pRocket->SetAbsVelocity(vecDirToPlayer * ROCKET_SPEED);
		pRocket->SetOwnerEntity(this);
		pRocket->SetDamage(sk_overlord_kick.GetFloat());
	}
