...
{ 
	if ( IsNPC() ) 
		return dynamic_cast<CAI_BaseNPC *>(this);
...
// Debugging
void CBaseEntity::DumpEFlags()
{
	Msg( "%s (%s) EFLAGS:\n", GetClassname(), GetEntityName() );
	if (m_iEFlags & EFL_KILLME) Msg( "EFL_KILLME\n" );
	if (m_iEFlags & EFL_DORMANT) Msg( "ELF_DORMANT\n" );
	if (m_iEFlags & EFL_NOCLIP_ACTIVE) Msg( "EFL_NOCLIP_ACTIVE\n" );
	if (m_iEFlags & EFL_SETTING_UP_BONES) Msg( "EFL_SETTING_UP_BONES\n" );
	if (m_iEFlags & EFL_KEEP_ON_RECREATE_ENTITIES) Msg( "EFL_KEEP_ON_RECREATE_ENTITIES\n" );
	if (m_iEFlags & EFL_HAS_PLAYER_CHILD) Msg( "EFL_HAS_PLAYER_CHILD\n" );
	if (m_iEFlags & EFL_DIRTY_SHADOWUPDATE) Msg( "EFL_DIRTY_SHADOWUPDATE\n" );
	if (m_iEFlags & EFL_NOTIFY) Msg( "EFL_NOTIFY\n" );
	if (m_iEFlags & EFL_FORCE_CHECK_TRANSMIT) Msg( "EFL_FORCE_CHECK_TRANSMIT\n" );
	if (m_iEFlags & EFL_BOT_FROZEN) Msg( "EFL_BOT_FROZEN\n" );
	if (m_iEFlags & EFL_SERVER_ONLY) Msg( "EFL_SERVER_ONLY\n" );
	if (m_iEFlags & EFL_NO_AUTO_EDICT_ATTACH) Msg( "EFL_NO_AUTO_EDICT_ATTACH\n" );
	if (m_iEFlags & EFL_DIRTY_ABSTRANSFORM) Msg( "EFL_DIRTY_ABSTRANSFORM\n" );
	if (m_iEFlags & EFL_DIRTY_ABSVELOCITY) Msg( "EFL_DIRTY_ABSVELOCITY\n" );
	if (m_iEFlags & EFL_DIRTY_SURROUNDING_COLLISION_BOUNDS) Msg( "EFL_DIRTY_SURROUNDING_COLLISION_BOUNDS\n" );
	if (m_iEFlags & EFL_DIRTY_SPATIAL_PARTITION) Msg( "EFL_DIRTY_SPATIAL_PARTITION\n" );
	if (m_iEFlags & EFL_IN_SKYBOX) Msg( "EFL_IN_SKYBOX\n" );
	if (m_iEFlags & EFL_USE_PARTITION_WHEN_NOT_SOLID) Msg( "EFL_USE_PARTITION_WHEN_NOT_SOLID\n" );
	if (m_iEFlags & EFL_TOUCHING_FLUID) Msg( "EFL_TOUCHING_FLUID\n" );
	if (m_iEFlags & EFL_IS_BEING_LIFTED_BY_BARNACLE) Msg( "EFL_IS_BEING_LIFTED_BY_BARNACLE\n" );
	if (m_iEFlags & EFL_NO_ROTORWASH_PUSH) Msg( "EFL_NO_ROTORWASH_PUSH\n" );
	if (m_iEFlags & EFL_NO_THINK_FUNCTION) Msg( "EFL_NO_THINK_FUNCTION\n" );
	if (m_iEFlags & EFL_NO_GAME_PHYSICS_SIMULATION) Msg( "EFL_NO_GAME_PHYSICS_SIMULATION\n" );
	if (m_iEFlags & EFL_CHECK_UNTOUCH) Msg( "EFL_CHECK_UNTOUCH\n" );
	if (m_iEFlags & EFL_DONTBLOCKLOS) Msg( "EFL_DONTBLOCKLOS\n" );
	if (m_iEFlags & EFL_DONTWALKON) Msg( "EFL_DONTWALKON\n" );
	if (m_iEFlags & EFL_NO_DISSOLVE) Msg( "EFL_NO_DISSOLVE\n" );
	if (m_iEFlags & EFL_NO_MEGAPHYSCANNON_RAGDOLL) Msg( "EFL_NO_MEGAPHYSCANNON_RAGDOLL\n" );
	if (m_iEFlags & EFL_NO_WATER_VELOCITY_CHANGE) Msg( "EFL_NO_WATER_VELOCITY_CHANGE\n" );
	if (m_iEFlags & EFL_NO_PHYSCANNON_INTERACTION) Msg( "EFL_NO_PHYSCANNON_INTERACTION\n" );
	if (m_iEFlags & EFL_NO_DAMAGE_FORCES) Msg( "EFL_NO_DAMAGE_FORCES\n" );
